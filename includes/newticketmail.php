...
$result1=$ost_wpdb->get_results("SELECT address FROM ".$prefix."user_email WHERE address = '".$wp_user_email_id."'");
if (count ($result1) > 0) {
	$row = current ($result1);	
} else {
	$ost_wpdb->query ("INSERT INTO ".$prefix."user_email (id, user_id, address) VALUES ('','".$usid."','".$wp_user_email_id."')");
	@$last_ost_user_email_id = $ost_wpdb->insert_id;
}

$result2=$ost_wpdb->get_results("SELECT default_email_id,name FROM ".$prefix."user WHERE default_email_id = '".@$last_ost_user_email_id."'");
if (count ($result2) > 0) {
	//$row = current ($result2);
	@$name=$result2->name;
	if(@$name=="")
	{
              if(@$last_ost_user_email_id!="")
                $wpdb->query("UPDATE ".$prefix."user SET name = ".$nam." WHERE default_email_id = '".@$last_ost_user_email_id."'");	
	}
} else {
	$ost_wpdb->query ("INSERT INTO ".$prefix."user (id, default_email_id, name, created, updated) VALUES ('','".$last_ost_user_email_id."', '".$nam."', '".$cre."', '".$cre."')
	");
	$last_ost_user_id = $ost_wpdb->insert_id;
        if($last_ost_user_id!="")    
	$ost_wpdb->query ("UPDATE ".$prefix."user_email SET user_id=$last_ost_user_id where id=$last_ost_user_email_id");
}
